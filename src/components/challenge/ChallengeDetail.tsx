...
const ChallengeDetail = ({ title, description, image_url, tip }: ChallengeDetailProps) => {
  return (
    <div className='flex flex-col justify-between items-center font-pretendard text-center h-[357px] mt-[5px]'>
      <h1 className='font-semibold text-[20px] desktop:text-[22px] mb-[14px] leading-[35px]'>{title}</h1>
      <div className='h-[342px] flex flex-col items-center justify-around'>
        <h3 className='font-medium text-[12px] mb-[17px] w-[345px] leading-[20px]'>{`"${description}"`}</h3>
        ...
        <p className='mt-[10px] font-medium text-[12px] mb-[9px] desktop:mb-[13px] w-[345px]'>
          <span className='text-primary-400'>💡Tip</span>
          <span className='text-neutral-500'> {tip}</span>
        </p>
      </div>
      <p className='absolute bottom-0 desktop:bottom-[20px] font-medium text-[10px] text-primary-400 leading-[35px]'>
